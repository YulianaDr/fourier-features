...
3. **Сравнение комбинаций признаков**  
   ...
   - многоклассовая классификация (MNIST): точность до 58%

4. **Сравнение с нейросетью**  
   Для задачи бинарной классификации (дефекты ткани) проведено сравнение предложенного метода (Фурье-признаки + Random Forest) с простой свёрточной нейронной сетью. Сравнение выполнено по точности и времени предсказания.
...
- На многоклассовой задаче MNIST точность не превышает 58% — метод не подходит для распознавания изолированных объектов.
...
|---------------------|---------------------|----------|
| Все вместе | 24 | 58.00% |
| Кольцевые + угловые | 20 | 57.33% |
| Только кольцевые | 16 | 52.50% |
| Кольцевые + статистические | 20 | 51.67% |
| Угловые + статистические | 8 | 44.00% |
| Только угловые | 4 | 39.00% |
| Только статистические | 4 | 33.33% |

**Анализ:**

- Точность на MNIST существенно ниже, чем на задаче с тканями (максимум 58%).
...
- Комбинация «все вместе» дала наилучший результат (58.00%), но он остаётся низким.
...
4. На задаче MNIST метод неэффективен (до 58%) — это его естественное ограничение.
