...
- $F(u,v)$ — комплексное число, содержит информацию об амплитуде (яркости волны) и фазе (её сдвиге)
...
*   **Фазовый спектр**, напротив, **чувствителен к сдвигу** (изменяется при смещении объекта даже на 1 пиксель), не инвариантен к повороту и не поддаётся усреднению по кольцам/секторам без потери смысла. Он не даёт значимого прироста точности в задачах классификации.
...
На визуализации спектра видно яркое пятно в центре — это низкие частоты (общая форма, фон). К краям спектр темнеет — высоких частот мало, что характерно для гладкой поверхности.
...
- **Кольца 1–5** (центр): низкие частоты — форма, фон. Чем выше значения, тем крупнее объект в кадре.
